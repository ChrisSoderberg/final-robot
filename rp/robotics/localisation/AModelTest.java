...
public class AModelTest {
	...
	public AModelTest(SimulatedRobot _robot, LineMap _lineMap,
	 ...
	 */
	@Test
	public void testProb(GridPositionDistribution grid, float junctionSep, int xInset, int yInset) {
		Pose pose = m_robot.getPose();
		int xPos = (int) ((pose.getX() - xInset)/junctionSep);
		int yPos = (int) ((pose.getY() - yInset)/junctionSep);

		if(grid.getProbability(xPos, yPos) != 0) {
			Assert.assertTrue((grid.getProbability(xPos, yPos)) != 0);
	...
	public static void main(String[] args) {
		...
		GridPositionDistribution grid = new GridPositionDistribution(gridMap);
		GridPositionDistribution prob = new GridPositionDistribution(grid);
		...
		AModelTest test = new AModelTest(robot,
				lineMap, gridMap, junctionSeparation);
		test.visualise();
		test.run();
		test.testProb(prob, junctionSeparation, xInset, yInset);
